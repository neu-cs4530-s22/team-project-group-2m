...
export function videoStatusUpdateHandler(_requestData: VideoStatusUpdateRequest) : ResponseEnvelope<Record<string, null>> {
  ...
  if (!townController?.getSessionByToken(_requestData.sessionToken)){
    return {
      isOK: false, response: {}, message: `Unable to update video with url: ${_requestData.videoStatus.url} and timestamp ${_requestData.videoStatus.timestamp}`,
    };
  }
  ...
  return {
    isOK: success,
    response: {},
    message: !success ? `Failed to update video with url: ${_requestData.videoStatus.url} and timestamp ${_requestData.videoStatus.timestamp}` : undefined,
  };
}
